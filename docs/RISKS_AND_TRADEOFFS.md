...
| A3 | **Scale stays within the [High-Level Design (HLD) section 2](HLD.md#2-context-constraints-and-sizing-assumptions) baseline** — 50k users, 150 req/s, 2M tasks | The launch topology. Stages 1–2 of the [evolution path](SCALABILITY.md#2-the-evolution-path) absorb 10×, so this degrades gracefully |
...
| A7 | Clients are a web single-page application (SPA) and a mobile app that we control | Shapes the token-storage decision |
| A8 | No file attachments in v1 | Object storage, virus scanning and signed web addresses (URLs) are absent |
...
If sharing arrives, the migration is: a `task_permissions` table, ownership checks replaced by permission lookups in `security/policy.py`, and every scoped query rewritten to join against it.

This is contained *because* the ownership check is centralised in the policy engine and the repository rather than scattered across handlers — which is a significant part of why it was centralised.

It remains a multi-week change touching the most sensitive code in the system, so I would want the requirement confirmed before building anything speculative for it.
...
| R2 | **Account takeover via credential stuffing** | High | High | **High** | Argon2id, breach-corpus rejection, dual-axis rate limiting, uniform error responses, anomaly alerting | Med — **multi-factor authentication (MFA) is the real fix and is deferred** |
| R3 | **A bad migration corrupts data** | Med | High | **High** | Backward-compatible migrations, migration-safety linter, staging rehearsal with production-shaped data, point-in-time recovery (PITR), quarterly restore drills | Low |
...
| R6 | A regional outage takes the system down | Low | High | Med | Multi-availability zone (AZ), cross-region snapshots, rehearsed rebuild runbook | **Accepted**: hours of downtime in a rare event |
...
| R8 | Supply-chain compromise via a dependency | Low | High | Med | Hash-pinned lockfile, `pip-audit`, Trivy, software bill of materials (SBOM), dependency review | Med — an irreducible ecosystem risk |
...
**R4** is worth naming because it is the failure that *looks like* something else. Every dashboard is green — central processing unit (CPU) low, queries fast — while requests queue for a connection. Teams respond by adding replicas, which makes it strictly worse. It is called out in [Scalability section 10](SCALABILITY.md#10-what-would-actually-break-first) and given a dedicated metric and alert precisely so it is diagnosed rather than discovered.

---

## 3. Bottlenecks

Ranked by when they are likely to be hit, as detailed in [Scalability section 10](SCALABILITY.md#10-what-would-actually-break-first).

| Rank | Bottleneck | Arrives at | Warning signal |
|---|---|---|---|
| 1 | Database connections | ~8–10 replicas | Pool utilisation > 70% |
| 2 | Unindexed queries | Data growth, not traffic | A single endpoint's 95th percentile (p95) diverging |
...
Full analysis in [Availability section 2](AVAILABILITY.md#2-single-points-of-failure). Summary:

| single point of failure (SPOF) | Status |
|---|---|
...
| Domain Name System (DNS) | Mitigated with multiple nameserver providers |
| continuous integration and continuous delivery (CI/CD) | Mitigated with a documented manual deploy path |
| Application replicas, Redis, object storage, email | Not single points of failure (SPOFs) by design |

---

## 5. Security Risks

Covered in [Security section 1](SECURITY.md#1-threat-model-first) and section 12. The gaps I am consciously carrying:
...
| **JSON Web Token (JWT) access tokens** | No database round trip on the hot path | Revocation is not instant; bounded to 15 minutes plus a denylist |
...
| **Soft delete** | Recoverable from accidental deletion | Every query carries a filter; unique constraints must be partial; a separate hard-delete path for General Data Protection Regulation (GDPR) |
| **Optimistic concurrency** | No locks held across user think-time | Clients must handle `409`/`412` |
| **Domain layer separate from object-relational mapping (ORM) models** | Fast, pure tests of the rules that matter | A mapping layer to maintain — **the most debatable choice in the design** |
